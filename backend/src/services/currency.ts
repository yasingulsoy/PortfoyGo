...
export class CurrencyService {
  ...
  static async getList(): Promise<CurrencyListItem[]> {
    const data = await this.request<any[]>('economy/currency/list');
  ...
  static async getRate(code: string): Promise<CurrencyRate | null> {
    const data = await this.request<any>('economy/currency/exchange-rate', { code });
    if (!data) return null;
    ...
    try {
      ...
      for (const r of results) {
        await client.query(
          ...
          [r.code, r.name, r.buying, r.selling, r.changeRate]
        );
      }
  ...
  /** DB'den oku */
  static async getFromDb(): Promise<CurrencyRate[]> {
    try {
      const result = await pool.query(
        'SELECT code, name, buying, selling, change_rate, updated_at FROM currency_rates ORDER BY code'
      );
  ...
  /** Tek döviz DB'den */
  static async getFromDbByCode(code: string): Promise<CurrencyRate | null> {
    try {
      const result = await pool.query(
        'SELECT code, name, buying, selling, change_rate, updated_at FROM currency_rates WHERE code = $1',
        [code.toUpperCase()]
      );
