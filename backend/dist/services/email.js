...
class EmailService {
    ...
    async sendVerificationCode(email, userId) {
        try {
            const verificationCode = this.generateVerificationCode();
            const expiresAt = new Date(Date.now() + 15 * 60 * 1000); // 15 dakika
            // Eski kodları temizle
            await database_1.default.query('DELETE FROM email_verifications WHERE email = $1 AND used = false', [email]);
            // Yeni kodu kaydet
            await database_1.default.query('INSERT INTO email_verifications (user_id, email, verification_code, expires_at) VALUES ($1, $2, $3, $4)', [userId, email, verificationCode, expiresAt]);
            // Email gönder
            const mailOptions = {
    ...
    async verifyCode(email, code) {
        try {
            const result = await database_1.default.query(`SELECT user_id, expires_at, used 
         ...
         LIMIT 1`, [email, code]);
            ...
            await database_1.default.query('UPDATE email_verifications SET used = true WHERE email = $1 AND verification_code = $2', [email, code]);
            // Kullanıcının email'ini doğrulanmış olarak işaretle
            await database_1.default.query('UPDATE users SET email_verified = true WHERE id = $1', [verification.user_id]);
            return {
                success: true,
    ...
    async sendPasswordResetCode(email) {
        try {
            ...
            const userResult = await database_1.default.query('SELECT id, username FROM users WHERE email = $1', [email]);
            ...
            await database_1.default.query('DELETE FROM email_verifications WHERE email = $1 AND used = false', [email]);
            // Yeni kodu kaydet
            await database_1.default.query('INSERT INTO email_verifications (user_id, email, verification_code, expires_at) VALUES ($1, $2, $3, $4)', [user.id, email, resetCode, expiresAt]);
            // Email gönder
            const mailOptions = {
                from: `"Sanal Yatırım" <${process.env.SMTP_USER}>`,
                to: email,
