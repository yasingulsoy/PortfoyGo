...
exports.StopLossService = void 0;
const database_1 = __importDefault(require("../config/database"));
const transaction_1 = require("./transaction");
class StopLossService {
    // Stop-loss emri oluştur
    static async createStopLoss(userId, data) {
        const client = await database_1.default.connect();
        try {
            await client.query('BEGIN');
            // Portföy öğesini kontrol et
            const portfolioResult = await client.query('SELECT * FROM portfolio_items WHERE id = $1 AND user_id = $2', [data.portfolio_item_id, userId]);
            ...
            const existingResult = await client.query('SELECT id FROM stop_loss_orders WHERE user_id = $1 AND portfolio_item_id = $2 AND status = $3', [userId, data.portfolio_item_id, 'active']);
            if (existingResult.rows.length > 0) {
                await client.query('ROLLBACK');
                return { success: false, message: 'Bu varlık için zaten aktif bir stop-loss emri var!' };
            }
            // Stop-loss emri oluştur
            const result = await client.query(`INSERT INTO stop_loss_orders 
    ...
    static async getStopLossOrders(userId) {
        try {
            const result = await database_1.default.query(`SELECT * FROM stop_loss_orders 
         WHERE user_id = $1 
         ORDER BY created_at DESC`, [userId]);
            const stopLossOrders = result.rows.map((row) => ({
                id: row.id,
                user_id: row.user_id,
    ...
    static async cancelStopLoss(userId, stopLossId) {
        const client = await database_1.default.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query('UPDATE stop_loss_orders SET status = $1 WHERE id = $2 AND user_id = $3 AND status = $4 RETURNING *', ['cancelled', stopLossId, userId, 'active']);
    ...
    static async checkAndTriggerStopLosses() {
        const client = await database_1.default.connect();
        try {
            await client.query('BEGIN');
            // Aktif stop-loss emirlerini getir
            const result = await client.query(`SELECT sl.*, pi.current_price, pi.name 
         ...
         WHERE sl.status = 'active'`);
            for (const order of result.rows) {
                ...
                if (currentPrice <= triggerPrice) {
                    ...
                    const sellResult = await transaction_1.TransactionService.sell(order.user_id, {
                        symbol: order.symbol,
                        quantity: parseFloat(order.quantity)
                    });
                    if (sellResult.success) {
                        // Stop-loss'u tetiklendi olarak işaretle
                        await client.query('UPDATE stop_loss_orders SET status = $1, triggered_at = CURRENT_TIMESTAMP WHERE id = $2', ['triggered', order.id]);
...
exports.StopLossService = StopLossService;
