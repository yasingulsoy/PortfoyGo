...
router.get('/', async (req, res) => {
    try {
        const rates = await currency_1.CurrencyService.getFromDb();
        res.json({
            success: true,
            data: rates.map(p => ({
                code: p.code,
...
router.get('/list', async (req, res) => {
    try {
        const list = await currency_1.CurrencyService.getList();
...
router.get('/:code', async (req, res) => {
    try {
        ...
        const rate = await currency_1.CurrencyService.getFromDbByCode(code);
        if (!rate) {
...
router.post('/refresh', async (req, res) => {
    try {
        const count = await currency_1.CurrencyService.fetchAndSaveToDb();
